...
def my_save_image_list(opath, dataroot, filelist):
    ...
    with open(opath, 'w') as f:
        for line in sourcefile:
            print(os.path.join(dataroot, line.split()[0]), file=f)
...
def rebuild_dataset(interactive=False, opt=None):
    ...
    # load old attributes
    if os.path.exists('datasets/%s/attributes.npz' % opt.dataset):
    ...
    my_save_image_list('datasets/%s/filelist.txt' % opt.dataset, opt.dataroot, opt.filelist)
    S = [os.path.join(opt.dataroot, x.split()[0].strip()) for x in open(opt.filelist).readlines()]
    A = [int(x.split()[1].strip()) for x in open(opt.filelist).readlines()]
    print('Found {} images.'.format(len(S)))
    T = [x for x in S if x not in old_filelist_revmap]
    print('Found {} new images.'.format(len(T)))

    # load history
    if os.path.exists('datasets/%s/history.npz' % opt.dataset):
        dedup_descriptors = npz.NpzLog('datasets/%s/history.npz' % opt.dataset, shape=(0, 1024), dtype=numpy.uint8, buflen=len(T))
    else:
    ...
    def process_dedup(ipath):
        ...
        if ipath not in dedup_descriptors:
            ...
            if ((u['n_buffer']+u['n_overflow']) % 10000) == 0:
                ...
                dedup_descriptors.write('datasets/%s/history.npz' % opt.dataset)
        else:
    ...
    with open('datasets/%s/attributes.npz' % opt.dataset, 'wb') as f:
        numpy.savez(f, scores=new_scores, landmarks=new_landmarks, filelist=new_filelist, attributes=new_attributes)
    dedup_descriptors.write('datasets/%s/history.npz' % opt.dataset)
    if interactive:
...
def build_url_list(opt):
    # list celeba, helen, megaface images
    # lookup original URLs for LFW-searched google images
    filelist=[x.strip() for x in open('datasets/%s/filelist.txt' % opt.dataset).readlines()]
...
if __name__=='__main__':
    ...
    parser.add_argument('--dataset', type=str, default='cacd', help='name of dataset')
    parser.add_argument('--dataroot', type=str, default='')
    parser.add_argument('--filelist', type=str, default='', help='file names and attributes')
    config = parser.parse_args()
    if config.dataroot.startswith('/'):
        raise TypeError('dataroot should be relative path')
    rebuild_dataset(interactive=False, opt=config)
    #build_url_list(config)
